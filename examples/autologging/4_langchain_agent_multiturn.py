...
def main():
    ...
    # Create in memory session handler to handle user sessions
    ...
    # Process a multiturn conversation - attach config so the session can be tracked
    ...
    new_request = "What about NYC?"
    print("New Request:\n", new_request)
    new_input = {"messages": [{"role": "user", "content": new_request}]}
    response = agent.invoke(input=new_input, config=config)
    print("Response:\n", response.get("messages", [])[-1].content)
